...
class ClusteringRunResponse(BaseModel):
    """Clustering run response with basic information."""
    ...
    algorithms: List[str] = Field(..., description="Algorithms requested")
    preprocessingConfig: Dict[str, Any] = Field(..., description="Preprocessing configuration")
    ...
    totalExecutionTime: Optional[float] = Field(None, description="Total execution time")
    results: Optional[Dict[str, Any]] = Field(None, description="Results data")
    bestAlgorithm: Optional[str] = Field(None, description="Best algorithm")
    bestScore: Optional[float] = Field(None, description="Best score")
