...
export const FileUpload: React.FC<FileUploadProps> = ({
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Page Header */}
      <PageHeader
        title="Spectral Analysis Platform"
        status="connected"
        stats={{
          samples: 890000,
          features: 17,
      ...
      <Box sx={{ flex: 1, p: 3, overflow: 'auto' }}>
        ...
        <Box sx={{ mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
            <CloudUpload sx={{ color: '#1976d2', fontSize: '1.5rem' }} />
            <Typography variant="h5" component="h2" sx={{ fontWeight: 600, color: '#1976d2' }}>
              Data Upload & Processing
            </Typography>
          </Box>
          <Typography variant="body2" color="text.secondary">
            Upload your CSV dataset to begin spectral analysis
          </Typography>
        </Box>
          ...
          <Card sx={{ mb: 3 }}>
            <CardContent>
              ...
              <Typography variant="subtitle1" gutterBottom sx={{ mt: 2 }}>
                Preview ({Math.min(maxPreviewRows, filePreview.totalRows)} of{' '}
                {filePreview.totalRows} rows)
              </Typography>
