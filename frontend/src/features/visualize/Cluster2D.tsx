 ...
 * 
 * Renders 2D scatter plot visualization of clustering results using Recharts.
 * Shows data points colored by cluster assignments with interactive features.
 */
  ...
  const { clusterData, clusterStats } = useMemo(() => {
    ...
    if (embeddings && embeddings.length > 0) {
      dataPoints = embeddings.map(point => ({
        x: point.x,
        y: point.y,
        cluster: labels ? labels[point.original_index] : 0,
      ...
      <Card>
        <CardContent>
          ...
          <Alert severity="error">
            Invalid data format. Please ensure points are in 2D format and labels match the number of points.
          </Alert>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      <CardContent>
        ...
        <ResponsiveContainer width={width || '100%'} height={height}>
          ...
          >
            ...
            <XAxis 
              ...
              label={{ value: effectiveXLabel, position: 'insideBottom', offset: -10 }}
            />
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
          Showing {points?.length || 0} points across {clusterStats.length} cluster{clusterStats.length !== 1 ? 's' : ''}
        </Typography>
      </CardContent>
    </Card>
  );
